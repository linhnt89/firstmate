...
#   (f) bare numeric pr= keeps the legacy GitHub pull-head fetch and stays current
set -u
...
test_bare_numeric_pr_meta_fetches_newer_pull_head() {
  local case_dir out stale_sha
  case_dir=$(make_case numeric-pr-fetch)
  stale_and_pr_commits "$case_dir"
  stale_sha=$(git -C "$case_dir/wt" rev-parse fm/task-x1)
  # Keep the fixture's local bare repository while giving the legacy numeric
  # target the forge origin it must use to infer GitHub.
  git -C "$case_dir/wt" remote set-url origin https://github.com/example/repo.git
  git -C "$case_dir/wt" config url."$case_dir/origin.git".insteadOf https://github.com/example/repo.git
  git -C "$case_dir/wt" push -q origin "pr-head-tmp:refs/pull/9/head"
  write_task_meta "$case_dir" "pr=9" "pr_head=$stale_sha"

  out=$(run_review_diff "$case_dir" task-x1 2> "$case_dir/stderr")

  assert_contains "$out" '+pr-fixed' \
    "numeric-pr-fetch: diff should use the newer live GitHub pull head"
  assert_not_contains "$out" 'stale-local' \
    "numeric-pr-fetch: diff must not use the stale local or recorded head"
  assert_not_contains "$(cat "$case_dir/stderr")" 'warning: review request head unavailable' \
    "numeric-pr-fetch: legacy bare-number fetch should succeed"
  pass "fm-review-diff preserves bare numeric pr= compatibility with a newer remote PR head"
}
