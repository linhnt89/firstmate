...
A local alignment conversation is disposable, while `bin/fm-alignment-session.sh` retains its validated report in the parent-owned project archive before cleanup; retention and cleanup bind to a reconciled project and archive snapshot, and incomplete abandonment is retained as non-promotable history.
