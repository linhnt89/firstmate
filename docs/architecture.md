...
Fresh local alignment sessions are the default captain-facing path: Firstmate resolves one project and one topic, and `fm-alignment-session.sh` leases a disposable session without adding a persistent Secondmate route.
...
Explicit backend-target sends stay unmarked, and direct human typing is conversational only for a captain-explicitly-requested fresh alignment session; ordinary implementation remains parent-routed.
...
A local alignment conversation is disposable, while `bin/fm-alignment-session.sh` retains its validated report in the parent-owned project archive before cleanup; launch delivery is separate from semantic readiness, and retention requires a current-epoch executor event folded by the parent against the last accepted project and archive snapshot.
A mutable session can publish refreshed context as pending reconciliation, then the executor emits the matching epoch through the existing parent status channel; stale or replayed events cannot advance the parent lifecycle record.
A completed outcome freezes its historical-inventory baseline so independent later archives do not make it unpromotable, while canonical project changes still require a revised session and explicit supersession.
Incomplete abandonment remains safe when stale and is retained as non-promotable history.
The existing `bin/fm-alignment.sh` direct pointer remains the non-default compatibility return channel for already-provisioned persistent Secondmates, and a parent-routed request preserves its correlation.
...
`no-mistakes` tasks run the full validation pipeline, `direct-PR` tasks open provider review requests without that pipeline, and `local-only` tasks stay local until firstmate performs an approved fast-forward merge.
...
`bin/fm-forge-capability-lib.sh` resolves the target project's provider and local write capability, so GitHub and GitLab tools and authentication are required only for the provider operation that needs them.
When a selected delivery path calls for a diff, `bin/fm-review-diff.sh` refreshes the authoritative base and, when task meta records `pr=`, always fetches the provider's review-request head by default (recorded `pr_head=` is only an offline fallback) before falling back to the local branch with a warning.
