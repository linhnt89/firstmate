...
A local alignment conversation is disposable, while `bin/fm-alignment-session.sh` retains its validated report in the parent-owned project archive before cleanup; launch delivery is separate from executor-authenticated semantic readiness, and retention requires the latter against the last acknowledged project and archive snapshot.
A mutable session can publish refreshed context as pending reconciliation, but only the bound executor-owned route can acknowledge it; parent callers cannot synthesize that acknowledgement with copied session identity fields.
