...
Use `bin/fm-alignment-session.sh start` to resolve one project and one topic, and use its `retain`, `inventory`, `retrieve`, `promote`, and `close` commands for the rest of the lifecycle.
