...
#   no-mistakes  implement -> /no-mistakes pipeline -> provider review request -> configured merge authority
#   direct-PR    implement -> push + open provider review request (gh-axi for GitHub, glab for GitLab) -> configured merge authority
...
A message with NO marker is unmarked direct input. Only when the captain explicitly initiates this Secondmate's focused pre-implementation alignment session may it use this non-default compatibility path; new local alignment uses a fresh project-scoped session from \`bin/fm-alignment-session.sh\`.
...
For direct-alignment compatibility, use the local report and completion commands in the pre-implementation alignment section below, not a detailed status transcript through the main firstmate.
...
For a captain-initiated direct local alignment with no marked request, an already-provisioned home may run \`bin/fm-alignment.sh start <alignment-id>\` and finish with \`bin/fm-alignment.sh complete-direct <alignment-id>\` after the report is complete.
...
This task ships **direct-PR**: you raise the provider review request yourself, without the no-mistakes pipeline.
The task is complete only when committed on your branch.
Determine the provider from the project's origin before delivery.
For GitHub, push the branch and open a pull request with \`gh-axi\`; for GitLab, push the branch and open a merge request with \`glab\`.
